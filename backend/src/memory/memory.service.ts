...
export class MemoryService {
  ...
  async create(createMemoryDto: CreateMemoryDto): Promise<any> {
    ...
    // Create relationships for linked memories
    for (const linkedMemoryId of linkedMemories) {
      await this.neo4jService.write(
        `MATCH (m1:Memory), (m2:Memory)
         WHERE id(m1) = $memoryId AND id(m2) = $linkedMemoryId
         CREATE (m1)-[:RELATED_TO]->(m2)`,
        { memoryId, linkedMemoryId: parseInt(linkedMemoryId) },
      );
    }
  ...
  async findAll(): Promise<any> {
    const result = await this.neo4jService.read(
      `MATCH (m:Memory)
       ...
       OPTIONAL MATCH (m)-[:RELATED_TO]->(lm:Memory)
  ...
  async update(id: string, updateMemoryDto: UpdateMemoryDto): Promise<any> {
    ...
    if (updateFields.length > 0) {
      await this.neo4jService.write(
        `MATCH (m:Memory) WHERE id(m) = $id SET ${updateFields.join(', ')}`,
        updateParams,
      );
    }
    ...
    if (people) {
      await this.neo4jService.write(
        `MATCH (m:Memory) WHERE id(m) = $id
         ...
         CREATE (m)-[:INVOLVES_PERSON]->(p)`,
        { id: parseInt(id), people: people.map((p) => ({ name: p.name })) },
      );
    }
    ...
    if (places) {
      await this.neo4jService.write(
        `MATCH (m:Memory) WHERE id(m) = $id
         ...
         CREATE (m)-[:OCCURRED_AT]->(pl)`,
        { id: parseInt(id), places: places.map((p) => ({ name: p.name })) },
      );
    }

    // Handle linked memories relationships
    if (linkedMemories) {
      await this.neo4jService.write(
        `MATCH (m1:Memory) WHERE id(m1) = $id
         OPTIONAL MATCH (m1)-[r:RELATED_TO]->(m2:Memory)
         DELETE r
         WITH m1
         UNWIND $linkedMemories AS linkedMemoryId
         MATCH (m2:Memory) WHERE id(m2) = linkedMemoryId
         CREATE (m1)-[:RELATED_TO]->(m2)`,
        {
          id: parseInt(id),
    ...
    if (photos) {
      await this.neo4jService.write(
        `MATCH (m:Memory) WHERE id(m) = $id
         SET m.photos = $photos`,
        { id: parseInt(id), photos: photos.map((p) => p.url) },
      );
    }

    // After updating, return the full memory object
    return await this.findOne(id);
    // ...existing code... (return handled above)
  }

  async remove(id: string): Promise<any> {
    await this.neo4jService.write(
      `MATCH (m:Memory) WHERE id(m) = $id DETACH DELETE m`,
      {
        id: parseInt(id),
      },
    );
